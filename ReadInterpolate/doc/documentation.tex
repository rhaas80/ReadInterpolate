...
reading interface via the \var{IO} paramters \var{filereader\_ID\_dir},
...
currrent grid, require that there are at least \var{cctk\_nghostzones} or
...
outer boundary. Then
\item if \var{read\_only\_timelevel\_0} is true (the default), reject all data
from time levels other than 0, then
...
higher than this,
\item{\code{read\_only\_timelevel\_0}} read only $t=0$ from files.
\end{description}
...
\code{AEILocalInterp}'s \texttt{Herminte} interpolator and forbid off centered
interpolation stencils or extrapolation.

\code{ReadInterpolate} uses \var{epsilon} as a fuzzyness
